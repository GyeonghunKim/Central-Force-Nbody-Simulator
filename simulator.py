...
class simulator:
    ...
    # 파라미터는 편의를 위해서, 또 일반적인 경우에 그러므로, 한 글자라고 가정했습니다.
    # force_string에서 상호작용하는 두 물체의 질량을 나타내는 문자는 m1, m2로 고정되어 사용되어야 합니다.'
    # 두 물체 사이의 거리는 x로 고정되어 있습니다.
    ...
    # 실제로 입자들을 상호작용에 따라서 운동 시키는 부분입니다.
    # 지정해준 횟수(iter)만큼 반복합니다.
    def exec(self, iter):
